class App {
        ...
        for (
            ...
            for (
                ...
                if (
                    ...
                    this.lastCorrectCell.row === row &&
                    this.lastCorrectCell.col === col
                ) {
    ...
    enterValue(value) {
        ...
        if (result.success) {
            ...
            /*
             * Briefly flash the cell green so the player
             * gets instant confirmation it's correct.
             */
    ...
    flashCorrect(row, col) {
        ...
        if (this.correctFlashTimeout) {

            clearTimeout(
                this.correctFlashTimeout
            );
    ...
    erase() {
        ...
        if (result.reason === "correct") {
            ...
            this.message(
                "That answer is correct, so it can't be erased. 🌿"
            );
        ...
        if (result.reason === "given") {

            this.message(
                "That's a fixed clue and can't be erased."
            );
    ...
    awardLevelScore() {
        ...
        if (this.levelRewarded) {
            return 0;
        }

        this.levelRewarded = true;

        const score =
            this.game.getScore();

        // Scores are always preserved. Negative levels simply
        // earn zero coins rather than taking coins away.
        const coinsEarned =
            Math.max(0, score);
        ...
        localStorage.setItem(
            "visualSudokuLastScore",
            String(score)
        );

        localStorage.setItem(
            "visualSudokuLastCoinsEarned",
            String(coinsEarned)
        );
